...
class ZerodhaTradingApp:
    ...
    def setup_trading_tab(self, notebook):
        ...
        paned_window = ttk.PanedWindow(trade_frame, orient=tk.HORIZONTAL)
        ...
        futures_frame = ttk.LabelFrame(left_frame, text="Available Futures Contracts (Live Prices)")
        futures_frame.pack(fill='both', expand=True, padx=5, pady=5)
        
        # Buttons for futures table
        futures_buttons_frame = ttk.Frame(futures_frame)
        ...
        # Futures table
        table_frame = ttk.Frame(futures_frame)
        ...
        order_frame = ttk.LabelFrame(right_frame, text="Place Orders")
        ...
        self.setup_single_transaction_tab(single_tab)
        
        # Setup buy/sell together tab
        self.setup_buy_sell_together_tab(pair_tab)
        
        # Orders log
        orders_log_frame = ttk.LabelFrame(right_frame, text="Orders Log")
        ...
        self.orders_text.pack(fill='both', expand=True, padx=5, pady=5)
    
    def setup_single_transaction_tab(self, parent):
        """Setup single transaction tab"""
        # Selection frame
        selection_frame = ttk.LabelFrame(parent, text="Contract Selection")
        selection_frame.pack(fill='x', padx=5, pady=5)
        
        ttk.Button(selection_frame, text="Select from Table", 
                  command=self.select_from_table_single).pack(side='left', padx=5, pady=5)
        ttk.Button(selection_frame, text="Clear Selection", 
                  command=self.clear_single_selection).pack(side='left', padx=5, pady=5)
        
        self.selected_single_text = scrolledtext.ScrolledText(selection_frame, height=4)
        self.selected_single_text.pack(fill='x', padx=5, pady=5)
        self.selected_single_text.insert(tk.END, "No contracts selected")
        
        # Order parameters
        params_frame = ttk.LabelFrame(parent, text="Order Parameters")
        params_frame.pack(fill='x', padx=5, pady=5)
        
        # Transaction type
        ttk.Label(params_frame, text="Transaction Type:").grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.transaction_type = ttk.Combobox(params_frame, values=["BUY", "SELL"])
        self.transaction_type.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.transaction_type.set("BUY")
        
        # Order type
        ttk.Label(params_frame, text="Order Type:").grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.order_type = ttk.Combobox(params_frame, values=["MARKET", "LIMIT"])
        self.order_type.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        self.order_type.set("MARKET")
        
        # Quantity type
        ttk.Label(params_frame, text="Quantity Type:").grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.quantity_type = ttk.Combobox(params_frame, values=["Fixed Quantity", "Lot Size"])
        self.quantity_type.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        self.quantity_type.set("Lot Size")
        
        # Quantity
        ttk.Label(params_frame, text="Quantity:").grid(row=3, column=0, padx=5, pady=5, sticky='w')
        self.quantity_entry = ttk.Entry(params_frame)
        ...
        self.price_entry = ttk.Entry(params_frame)
        self.price_entry.grid(row=4, column=1, padx=5, pady=5, sticky='ew')
        ...
        ttk.Button(order_buttons_frame, text="Place Orders with Real-time Prices", 
                  command=self.place_single_orders).pack(side='left', padx=5)
        ttk.Button(order_buttons_frame, text="Validate Selection", 
                  command=self.validate_single_selection).pack(side='left', padx=5)
        
        # Configure grid weights
        params_frame.columnconfigure(1, weight=1)
    
    def setup_buy_sell_together_tab(self, parent):
        """Setup buy and sell together tab with separate contract selection"""
        ...
        buy_contracts_frame = ttk.LabelFrame(buy_selection_frame, text="BUY Contracts Selection")
        buy_contracts_frame.pack(fill='both', expand=True, padx=5, pady=5)
        
        ttk.Button(buy_contracts_frame, text="Select BUY Contracts", 
                  command=self.select_buy_contracts).pack(padx=5, pady=5)
        ttk.Button(buy_contracts_frame, text="Clear BUY Selection", 
                  command=self.clear_buy_selection).pack(padx=5, pady=5)
        
        self.selected_buy_text = scrolledtext.ScrolledText(buy_contracts_frame, height=8)
        self.selected_buy_text.pack(fill='both', expand=True, padx=5, pady=5)
        self.selected_buy_text.insert(tk.END, "No BUY contracts selected")
        ...
        self.buy_order_type = ttk.Combobox(buy_params_frame, values=["MARKET", "LIMIT"])
        self.buy_order_type.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.buy_order_type.set("MARKET")
        
        ttk.Label(buy_params_frame, text="Quantity Type:").grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.buy_quantity_type = ttk.Combobox(buy_params_frame, values=["Fixed Quantity", "Lot Size"])
        self.buy_quantity_type.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        self.buy_quantity_type.set("Lot Size")
        
        ttk.Label(buy_params_frame, text="Quantity:").grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.buy_quantity_entry = ttk.Entry(buy_params_frame)
        self.buy_quantity_entry.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        self.buy_quantity_entry.insert(0, "1")
        
        ttk.Label(buy_params_frame, text="Price (for LIMIT):").grid(row=3, column=0, padx=5, pady=5, sticky='w')
        self.buy_price_entry = ttk.Entry(buy_params_frame)
        self.buy_price_entry.grid(row=3, column=1, padx=5, pady=5, sticky='ew')
        self.buy_price_entry.insert(0, "0")
        
        # SELL Contracts Selection
        sell_contracts_frame = ttk.LabelFrame(sell_selection_frame, text="SELL Contracts Selection")
        sell_contracts_frame.pack(fill='both', expand=True, padx=5, pady=5)
        
        ttk.Button(sell_contracts_frame, text="Select SELL Contracts", 
                  command=self.select_sell_contracts).pack(padx=5, pady=5)
        ttk.Button(sell_contracts_frame, text="Clear SELL Selection", 
                  command=self.clear_sell_selection).pack(padx=5, pady=5)
        
        self.selected_sell_text = scrolledtext.ScrolledText(sell_contracts_frame, height=8)
        self.selected_sell_text.pack(fill='both', expand=True, padx=5, pady=5)
        self.selected_sell_text.insert(tk.END, "No SELL contracts selected")
        ...
        self.sell_order_type = ttk.Combobox(sell_params_frame, values=["MARKET", "LIMIT"])
        self.sell_order_type.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.sell_order_type.set("MARKET")
        
        ttk.Label(sell_params_frame, text="Quantity Type:").grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.sell_quantity_type = ttk.Combobox(sell_params_frame, values=["Fixed Quantity", "Lot Size"])
        self.sell_quantity_type.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        self.sell_quantity_type.set("Lot Size")
        
        ttk.Label(sell_params_frame, text="Quantity:").grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.sell_quantity_entry = ttk.Entry(sell_params_frame)
        self.sell_quantity_entry.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        self.sell_quantity_entry.insert(0, "1")
        
        ttk.Label(sell_params_frame, text="Price (for LIMIT):").grid(row=3, column=0, padx=5, pady=5, sticky='w')
        self.sell_price_entry = ttk.Entry(sell_params_frame)
        self.sell_price_entry.grid(row=3, column=1, padx=5, pady=5, sticky='ew')
        ...
        ttk.Button(combined_button_frame, text="Place BUY & SELL Orders with Real-time Prices", 
                  command=self.place_buy_sell_orders).pack(pady=5)
        
        # Configure grid weights
        buy_params_frame.columnconfigure(1, weight=1)
        sell_params_frame.columnconfigure(1, weight=1)
    
    def select_from_table_single(self):
        """Select contracts for single transaction from table"""
        self.open_selection_window('single')
    
    def select_buy_contracts(self):
        """Select contracts for BUY orders"""
        self.open_selection_window('buy')
    
    def select_sell_contracts(self):
        """Select contracts for SELL orders"""
        self.open_selection_window('sell')
    
    def open_selection_window(self, order_type):
        """Open selection window for BUY or SELL contracts"""
        selection_window = tk.Toplevel(self.root)
        selection_window.title(f"Select Contracts for {order_type.upper()} Orders")
        ...
        # Populate tree with current futures data
        ...
        # Bind click event
        select_tree.bind('<Button-1>', lambda e: self.on_selection_tree_click(e, select_tree, order_type))
        ...
        ttk.Button(button_frame, text="Select All", 
                  command=lambda: self.select_all_in_tree(select_tree, order_type)).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Clear All", 
                  command=lambda: self.clear_all_in_tree(select_tree, order_type)).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Confirm Selection", 
                  command=lambda: self.confirm_selection(select_tree, selection_window, order_type)).pack(side='right', padx=5)
    
    def on_selection_tree_click(self, event, tree, order_type):
        """Handle clicks on selection tree"""
        item = tree.identify_row(event.y)
        column = tree.identify_column(event.x)
        
        if item and column == '#1':  # Clicked on the Select column
    ...
    def select_all_in_tree(self, tree, order_type):
        """Select all contracts in tree"""
    ...
    def clear_all_in_tree(self, tree, order_type):
    ...
    def confirm_selection(self, tree, window, order_type):
        """Confirm selection for BUY or SELL orders"""
        if order_type == 'single':
            self.update_single_selection_display()
            message = f"{len(self.selected_single_futures)} contracts selected for trading"
        elif order_type == 'buy':
            self.update_buy_selection_display()
            message = f"{len(self.selected_buy_futures)} contracts selected for BUY orders"
        else:  # sell
            self.update_sell_selection_display()
            message = f"{len(self.selected_sell_futures)} contracts selected for SELL orders"
        
        window.destroy()
        messagebox.showinfo("Selection Complete", message)
    
    def update_single_selection_display(self):
        """Update single selection display"""
        self.selected_single_text.delete(1.0, tk.END)
        ...
        for symbol, details in self.selected_single_futures.items():
            self.selected_single_text.insert(tk.END, 
                f"Symbol: {symbol}\n"
                f"Name: {details['name']}\n"
    ...
    def update_buy_selection_display(self):
        """Update BUY selection display"""
        self.selected_buy_text.delete(1.0, tk.END)
        ...
        for symbol, details in self.selected_buy_futures.items():
            self.selected_buy_text.insert(tk.END, 
                f"Symbol: {symbol}\n"
                f"Name: {details['name']}\n"
                f"Expiry: {details['expiry']}\n"
                f"Lot Size: {details['lot_size']}\n"
    ...
    def update_sell_selection_display(self):
        """Update SELL selection display"""
        self.selected_sell_text.delete(1.0, tk.END)
        ...
        for symbol, details in self.selected_sell_futures.items():
            self.selected_sell_text.insert(tk.END, 
                f"Symbol: {symbol}\n"
                f"Name: {details['name']}\n"
                f"Expiry: {details['expiry']}\n"
                f"Lot Size: {details['lot_size']}\n"
    ...
    def clear_single_selection(self):
        """Clear single selection"""
        self.selected_single_futures.clear()
        self.update_single_selection_display()
    
    def clear_buy_selection(self):
        """Clear BUY selection"""
        self.selected_buy_futures.clear()
        self.update_buy_selection_display()
    
    def clear_sell_selection(self):
        """Clear SELL selection"""
        self.selected_sell_futures.clear()
        self.update_sell_selection_display()
    
    def validate_single_selection(self):
        """Validate single selection"""
        if not self.selected_single_futures:
            messagebox.showwarning("Warning", "No contracts selected")
            return
        
        messagebox.showinfo("Selection Valid", 
                          f"{len(self.selected_single_futures)} contracts selected and ready for trading")
    ...
    def show_real_time_price_window(self, symbols, transaction, order_type, quantity_type, base_quantity, price):
        """Show real-time price window before order placement"""
        price_window = tk.Toplevel(self.root)
        price_window.title("Real-time Prices - Confirm Order")
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Real-time Prices (Updating every 1 second)", 
                               font=('Arial', 12, 'bold'))
        ...
        def place_orders_now():
            ...
            Thread(
                target=self.execute_single_orders_with_current_prices,
        ...
        def cancel_orders():
            ...
            if price_window in self.real_time_windows:
                self.real_time_windows.remove(price_window)
            self.log_message("Order placement cancelled by user")
        
        ttk.Button(button_frame, text="Place Orders Now", 
        ...
        self.update_price_display(price_labels, price_window)
    
    def update_price_display(self, price_labels, window):
        ...
        if window.winfo_exists():
            window.after(1000, lambda: self.update_price_display(price_labels, window))
    
    def place_single_orders(self):
        """Place single transaction orders with real-time price updates"""
        if not self.is_logged_in:
            messagebox.showerror("Error", "Please login first")
            return
        ...
        try:
            ...
            base_quantity = int(self.quantity_entry.get())
            ...
            self.show_real_time_price_window(symbols, transaction, order_type, quantity_type, base_quantity, price)
            
        except ValueError as e:
            messagebox.showerror("Error", "Please enter valid quantity and price values")
        except Exception as e:
            self.log_message(f"Error starting order placement: {e}")
    
    def place_buy_sell_orders(self):
        """Place both BUY and SELL orders with real-time price updates"""
        if not self.is_logged_in:
            messagebox.showerror("Error", "Please login first")
            return
        
        if not self.selected_buy_futures and not self.selected_sell_futures:
            messagebox.showwarning("Warning", "No contracts selected for BUY or SELL")
            return
        
        try:
            # Buy order parameters
            buy_order_type = self.buy_order_type.get()
            buy_quantity_type = self.buy_quantity_type.get()
            buy_quantity = int(self.buy_quantity_entry.get())
            buy_price = float(self.buy_price_entry.get()) if self.buy_price_entry.get() and float(self.buy_price_entry.get()) > 0 else 0
            
            # Sell order parameters
            sell_order_type = self.sell_order_type.get()
            sell_quantity_type = self.sell_quantity_type.get()
            sell_quantity = int(self.sell_quantity_entry.get())
            sell_price = float(self.sell_price_entry.get()) if self.sell_price_entry.get() and float(self.sell_price_entry.get()) > 0 else 0
            ...
            self.show_buy_sell_real_time_window(
            ...
            self.log_message(f"Error starting buy/sell order placement: {e}")
    
    def show_buy_sell_real_time_window(self, buy_symbols, sell_symbols, buy_order_type, buy_quantity_type, 
                                     buy_quantity, buy_price, sell_order_type, sell_quantity_type, 
                                     sell_quantity, sell_price):
        """Show real-time price window for buy/sell orders"""
        price_window = tk.Toplevel(self.root)
        price_window.title("Real-time Prices - Confirm BUY & SELL Orders")
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Real-time Prices for BUY & SELL Orders", 
                               font=('Arial', 12, 'bold'))
        title_label.pack(pady=10)
        ...
        # BUY Frame
        buy_frame = ttk.LabelFrame(paned_window, text="BUY Contracts")
        paned_window.add(buy_frame, weight=1)
        
        # SELL Frame
        sell_frame = ttk.LabelFrame(paned_window, text="SELL Contracts")
        ...
        def place_orders_now():
            ...
            Thread(
                target=self.execute_buy_sell_orders_with_current_prices,
                args=(
                    buy_order_type, buy_quantity_type, buy_quantity, buy_price,
                    sell_order_type, sell_quantity_type, sell_quantity, sell_price
                ),
        ...
        def cancel_orders():
            ...
            if price_window in self.real_time_windows:
                self.real_time_windows.remove(price_window)
            self.log_message("BUY/SELL order placement cancelled by user")
        
        ttk.Button(button_frame, text="Place BUY & SELL Orders", 
                  command=place_orders_now).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Cancel", 
                  command=cancel_orders).pack(side='left', padx=5)
        
        # Start price updates
        self.update_buy_sell_price_display(buy_price_labels, sell_price_labels, price_window)
    
    def update_buy_sell_price_display(self, buy_labels, sell_labels, window):
        """Update BUY and SELL price labels"""
        if not window.winfo_exists():
            return
        ...
        try:
            ...
            print(f"Error updating buy/sell price display: {e}")
        
        # Schedule next update
        if window.winfo_exists():
            window.after(1000, lambda: self.update_buy_sell_price_display(buy_labels, sell_labels, window))
    
    def execute_single_orders_with_current_prices(self, transaction, order_type, quantity_type, base_quantity, price):
        """Execute single transaction orders using the latest prices"""
        orders_placed = 0
        total_orders = len(self.selected_single_futures)
        
        self.log_message(f"Starting to place {total_orders} {transaction} orders with real-time prices...")
            ...
            try:
                ...
                if not current_price:
                    ...
                    try:
                        ...
                        self.log_message(f"Fetched current LTP for {symbol}: {current_price}")
                    except:
                        self.log_message(f"❌ Could not fetch LTP for {symbol}, skipping...")
                        continue
                ...
                if order_type == "LIMIT":
                    ...
                    self.log_message(f"Limit order for {symbol}: Using price {final_price:.2f} (Current LTP: {current_price:.2f})")
                ...
                orders_placed += 1
                self.log_message(f"✅ {transaction} Order {orders_placed}/{total_orders}: {symbol} {quantity} @ {final_price if order_type == 'LIMIT' else 'MARKET'} - ID: {order_id}")
                
                # Small delay between orders
                time.sleep(1)
                
            except Exception as e:
                self.log_message(f"❌ Failed to place {transaction} order for {symbol}: {e}")
        
        self.log_message(f"{transaction} order placement completed: {orders_placed}/{total_orders} successful")
        
        # Show summary
        def show_summary():
            if orders_placed == total_orders:
                messagebox.showinfo("Success", f"All {orders_placed} {transaction} orders placed successfully!")
            else:
                messagebox.showwarning("Partial Success", 
                                     f"{orders_placed} out of {total_orders} {transaction} orders placed successfully")
        
        self.root.after(0, show_summary)
    
    def execute_buy_sell_orders_with_current_prices(self, buy_order_type, buy_quantity_type, buy_quantity, buy_price,
                                                  sell_order_type, sell_quantity_type, sell_quantity, sell_price):
        """Execute both BUY and SELL orders using the latest prices"""
        total_buy_orders = len(self.selected_buy_futures)
        total_sell_orders = len(self.selected_sell_futures)
        buy_orders_placed = 0
        sell_orders_placed = 0
        
        self.log_message(f"Starting to place {total_buy_orders} BUY and {total_sell_orders} SELL orders with real-time prices...")
        
        # Place BUY orders
        if total_buy_orders > 0:
            self.log_message("=== PLACING BUY ORDERS ===")
            for symbol, details in self.selected_buy_futures.items():
                try:
                    # Get latest price
                    current_price = self.current_prices.get(symbol)
                    if not current_price:
                        try:
                            ...
                            self.log_message(f"❌ Could not fetch LTP for BUY {symbol}, skipping...")
                            continue
                    ...
                    if buy_order_type == "LIMIT":
                        if buy_price == 0:
                            final_price = current_price * 0.995  # 0.5% below LTP
                        self.log_message(f"BUY Limit order for {symbol}: Using price {final_price:.2f} (Current LTP: {current_price:.2f})")
                    ...
                    buy_orders_placed += 1
                    self.log_message(f"✅ BUY Order {buy_orders_placed}/{total_buy_orders}: {symbol} {quantity} @ {final_price if buy_order_type == 'LIMIT' else 'MARKET'} - ID: {order_id}")
                    
                    time.sleep(1)
                    
                except Exception as e:
                    self.log_message(f"❌ Failed to place BUY order for {symbol}: {e}")
        
        # Place SELL orders
        if total_sell_orders > 0:
            self.log_message("=== PLACING SELL ORDERS ===")
            for symbol, details in self.selected_sell_futures.items():
                try:
                    # Get latest price
                    current_price = self.current_prices.get(symbol)
                    if not current_price:
                        try:
                            ...
                            self.log_message(f"❌ Could not fetch LTP for SELL {symbol}, skipping...")
                            continue
                    ...
                    if sell_order_type == "LIMIT":
                        if sell_price == 0:
                            final_price = current_price * 1.005  # 0.5% above LTP
                        self.log_message(f"SELL Limit order for {symbol}: Using price {final_price:.2f} (Current LTP: {current_price:.2f})")
                    ...
                    self.log_message(f"✅ SELL Order {sell_orders_placed}/{total_sell_orders}: {symbol} {quantity} @ {final_price if sell_order_type == 'LIMIT' else 'MARKET'} - ID: {order_id}")
                    
                    time.sleep(1)
                    
                except Exception as e:
                    self.log_message(f"❌ Failed to place SELL order for {symbol}: {e}")
        
        # Final summary
        self.log_message("=== ORDER PLACEMENT SUMMARY ===")
        self.log_message(f"BUY Orders: {buy_orders_placed}/{total_buy_orders} successful")
        self.log_message(f"SELL Orders: {sell_orders_placed}/{total_sell_orders} successful")
        
        def show_final_summary():
            messagebox.showinfo(
                "Buy & Sell Orders Completed",
                f"BUY Orders: {buy_orders_placed}/{total_buy_orders} successful\n"
                f"SELL Orders: {sell_orders_placed}/{total_sell_orders} successful"
            )
    ...
    def refresh_futures_table(self):
        ...
        try:
            ...
            self.log_message(f"Loaded {len(futures)} futures contracts")
            ...
            self.log_message(f"Error refreshing futures table: {e}")
    ...
    def start_futures_live_data(self):
        ...
        self.log_message("Started live prices for futures table")
    
    def stop_futures_live_data(self):
        """Stop live data updates for futures table"""
        self.futures_data_running = False
        self.log_message("Stopped live prices for futures table")
            ...
            try:
                ...
                for i in range(0, len(instruments), batch_size):
                    ...
                    try:
                        ...
                        self.log_message(f"Error updating batch {i//batch_size + 1}: {e}")
                
                time.sleep(3)  # Update every 3 seconds
                
            except Exception as e:
                self.log_message(f"Error in futures live data update: {e}")
                time.sleep(10)
    ...
    def start_background_tasks(self):
        ...
        # Auto-refresh futures table after login
        if self.is_logged_in:
            self.root.after(2000, self.refresh_futures_table)  # Refresh after 2 seconds

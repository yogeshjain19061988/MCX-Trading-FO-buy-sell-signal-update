...
class ZerodhaTradingApp:
    def __init__(self, root):
        ...
        # ===== NEW: CE/PE AUTOMATIC LIMIT PRICE SETTINGS =====
        self.ce_limit_settings = {
            'auto_limit_enabled': True,
            'buy_tolerance_percent': 0.5,  # 0.5% below market for BUY
            'sell_tolerance_percent': 0.5,  # 0.5% above market for SELL
            'min_price_tick': 0.05,  # Minimum price tick
            'max_price_adjustment': 2.0,  # Maximum 2% adjustment
            'use_market_order_fallback': True  # Fallback to market if limit fails
        }
        
        self.pe_limit_settings = {
            'auto_limit_enabled': True,
            'buy_tolerance_percent': 0.5,  # 0.5% below market for BUY
            'sell_tolerance_percent': 0.5,  # 0.5% above market for SELL
            'min_price_tick': 0.05,
            'max_price_adjustment': 2.0,
            'use_market_order_fallback': True
        }
        # =====================================================
    ...
    def execute_futures_single_orders_with_current_prices(self, transaction, order_type, quantity_type, base_quantity, price):
        ...
        for symbol, details in self.selected_single_futures.items():
            try:
                # ===== NEW: SAFETY CHECK =====
                # Calculate quantity first for validation
        ...
        # Update protection status after orders

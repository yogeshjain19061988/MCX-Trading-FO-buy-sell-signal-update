...
class ZerodhaTradingApp:
    def __init__(self, root):
        ...
        self.selected_buy_futures = {}  # Store selected futures for BUY orders
        self.selected_sell_futures = {}  # Store selected futures for SELL orders
        self.selected_single_futures = {}  # Store selected futures for single orders
    ...
    def setup_single_transaction_tab(self, parent):
        ...
        ttk.Button(order_buttons_frame, text="Place Orders", 
    ...
    def setup_buy_sell_together_tab(self, parent):
        ...
        ttk.Button(combined_button_frame, text="Place BUY & SELL Orders Together", 
    ...
    def place_single_orders(self):
        """Place single transaction orders"""
        if not self.is_logged_in:
            messagebox.showerror("Error", "Please login first")
            return
        ...
        try:
            ...
            threading.Thread(
                target=self.execute_single_orders,
    ...
    def place_buy_sell_orders(self):
        """Place both BUY and SELL orders with separate contract selection"""
        if not self.is_logged_in:
            messagebox.showerror("Error", "Please login first")
            return
        ...
        try:
            ...
            confirm = messagebox.askyesno(
                "Confirm Buy & Sell Orders",
                f"Place {len(self.selected_buy_futures)} BUY orders and {len(self.selected_sell_futures)} SELL orders?\n\n"
                f"BUY Contracts: {len(self.selected_buy_futures)}\n"
                f"BUY: {buy_order_type} orders, Quantity: {buy_quantity} ({buy_quantity_type})\n\n"
                f"SELL Contracts: {len(self.selected_sell_futures)}\n"
                f"SELL: {sell_order_type} orders, Quantity: {sell_quantity} ({sell_quantity_type})"
            )
            ...
            # Start order placement in background thread
            threading.Thread(
                target=self.execute_buy_sell_orders_separate,
                args=(
    ...
    def execute_single_orders(self, transaction, order_type, quantity_type, base_quantity, price):
        """Execute single transaction orders"""
        orders_placed = 0
        total_orders = len(self.selected_single_futures)
        
        self.log_message(f"Starting to place {total_orders} {transaction} orders...")
        ...
        for symbol, details in self.selected_single_futures.items():
            try:
                ...
                try:
                    ...
                    self.log_message(f"Current LTP for {symbol}: {current_ltp}")
                except:
                    current_ltp = 0
                    self.log_message(f"Could not fetch LTP for {symbol}")
                
                # Adjust price for limit orders if needed
                final_price = price
                if order_type == "LIMIT" and price == 0 and current_ltp > 0:
                    # Set limit price based on LTP
                    if transaction == "BUY":
                        final_price = current_ltp * 0.995  # 0.5% below LTP
                    else:
                        final_price = current_ltp * 1.005  # 0.5% above LTP
                ...
                orders_placed += 1
                self.log_message(f"✅ {transaction} Order {orders_placed}/{total_orders}: {symbol} {quantity} - ID: {order_id}")
    ...
    def execute_buy_sell_orders_separate(self, buy_order_type, buy_quantity_type, buy_quantity, buy_price,
                                       sell_order_type, sell_quantity_type, sell_quantity, sell_price):
        """Execute both BUY and SELL orders with separate contract selection"""
        total_buy_orders = len(self.selected_buy_futures)
        total_sell_orders = len(self.selected_sell_futures)
        buy_orders_placed = 0
        sell_orders_placed = 0
        
        self.log_message(f"Starting to place {total_buy_orders} BUY and {total_sell_orders} SELL orders...")
        ...
        if total_buy_orders > 0:
            ...
            for symbol, details in self.selected_buy_futures.items():
                try:
                    ...
                    if buy_order_type == "LIMIT" and buy_price == 0 and current_ltp > 0:
                        final_price = current_ltp * 0.995  # 0.5% below LTP
                    ...
                    buy_orders_placed += 1
                    self.log_message(f"✅ BUY Order {buy_orders_placed}/{total_buy_orders}: {symbol} {quantity} - ID: {order_id}")
        ...
        if total_sell_orders > 0:
            ...
            for symbol, details in self.selected_sell_futures.items():
                try:
                    ...
                    if sell_order_type == "LIMIT" and sell_price == 0 and current_ltp > 0:
                        final_price = current_ltp * 1.005  # 0.5% above LTP
                    ...
                    self.log_message(f"✅ SELL Order {sell_orders_placed}/{total_sell_orders}: {symbol} {quantity} - ID: {order_id}")

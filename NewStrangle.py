...
class ZerodhaTradingApp:
    def __init__(self, root):
        ...
        # Spread monitoring
        self.spread_monitor_running = False
        self.spread_smiley_window = None
    ...
    def setup_gui(self):
        ...
        # Market Data Tab (with basic spread monitoring)
        self.setup_market_data_tab(notebook)
        
        # Spread Analysis Tab (advanced)
        self.setup_spread_analysis_tab(notebook)
    ...
    def setup_market_data_tab(self, notebook):
        ...
        # Add spread monitoring button
        spread_frame = ttk.Frame(market_frame)
        spread_frame.pack(fill='x', padx=10, pady=5)
        
        ttk.Button(spread_frame, text="Monitor Futures Spread", 
                  command=self.start_spread_monitoring).pack(side='left', padx=5)
        ttk.Button(spread_frame, text="Stop Spread Monitor", 
                  command=self.stop_spread_monitoring).pack(side='left', padx=5)
        
        # Spread configuration
        ttk.Label(spread_frame, text="Base Symbol:").pack(side='left', padx=5)
        self.spread_base_symbol = ttk.Combobox(spread_frame, width=10, values=["GOLD", "SILVER", "CRUDEOIL"])
        self.spread_base_symbol.pack(side='left', padx=5)
        self.spread_base_symbol.set("GOLD")
    ...
    # ============= SPREAD MONITORING FUNCTIONS =============

    def calculate_futures_spread(self, contract1, contract2):
        """Calculate spread between two futures contracts"""
        try:
            # Get current prices for both contracts
            quote1 = self.kite.quote(f"MCX:{contract1}")
            quote2 = self.kite.quote(f"MCX:{contract2}")
            
            if quote1 and quote2:
                price1 = quote1[f"MCX:{contract1}"]['last_price']
                price2 = quote2[f"MCX:{contract2}"]['last_price']
                
                # Calculate spread (near month - far month)
            ...
            self.log_message(f"Error calculating spread: {e}")
        return None

    def start_spread_monitoring(self):
        """Start monitoring futures spreads"""
        if not self.is_logged_in:
            messagebox.showerror("Error", "Please login first")
            return
        
        self.spread_monitor_running = True
        
        # Get contracts for selected symbol
        base_symbol = self.spread_base_symbol.get()
        contracts = self.get_monthly_contracts(base_symbol)
        
        if len(contracts) < 2:
            messagebox.showerror("Error", f"Need at least 2 contracts for {base_symbol}")
            return
        
        # Use first two contracts (current and next month)
        self.spread_contracts = contracts[:2]
        
        # Start monitoring thread
        threading.Thread(target=self.monitor_spread_continuously, daemon=True).start()
        
        # Show smiley window
        self.show_spread_smiley_window()
        
        self.log_message(f"Started spread monitoring for {base_symbol}: {self.spread_contracts[0]} vs {self.spread_contracts[1]}")

    def stop_spread_monitoring(self):
        """Stop spread monitoring"""
        self.spread_monitor_running = False
        if self.spread_smiley_window:
            self.spread_smiley_window.destroy()
            self.spread_smiley_window = None
        self.log_message("Stopped spread monitoring")

    def monitor_spread_continuously(self):
        """Continuously monitor spread and update smiley"""
        update_interval = 2  # seconds
        
        while self.spread_monitor_running and self.is_logged_in:
            try:
                spread_data = self.calculate_futures_spread(
                    self.spread_contracts[0],
                    self.spread_contracts[1]
                )
                
                if spread_data:
                    # Update smiley color based on spread
                    self.update_spread_smiley(spread_data['spread'], spread_data['spread_percentage'])
                    
                    # Log spread information
                    self.log_message(
                        f"Spread {self.spread_contracts[0]}-{self.spread_contracts[1]}: "
                        f"{spread_data['spread']:.2f} ({spread_data['spread_percentage']:.2f}%) | "
                        f"Prices: {spread_data['price1']:.2f}/{spread_data['price2']:.2f}"
                    )
                
                time.sleep(update_interval)
                
            except Exception as e:
                self.log_message(f"Error in spread monitoring: {e}")
                time.sleep(5)

    def show_spread_smiley_window(self):
        """Show smiley window for spread indication"""
        if self.spread_smiley_window is not None and self.spread_smiley_window.winfo_exists():
            self.spread_smiley_window.destroy()
        
        self.spread_smiley_window = tk.Toplevel(self.root)
        self.spread_smiley_window.title("Futures Spread Smiley")
        self.spread_smiley_window.geometry("300x250")
        self.spread_smiley_window.resizable(False, False)
        
        # Make window stay on top
        self.spread_smiley_window.attributes('-topmost', True)
        
        # Center window
        self.center_window(self.spread_smiley_window)
        
        # Main frame
        main_frame = ttk.Frame(self.spread_smiley_window)
        main_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Title
        title_label = ttk.Label(
            main_frame, 
            text=f"Spread Monitor: {self.spread_base_symbol.get()}", 
            font=('Arial', 12, 'bold')
        )
        title_label.pack(pady=5)
        
        # Smiley label (will be updated)
        self.smiley_label = tk.Label(
            main_frame,
            text="😐",  # Neutral face
            font=('Arial', 72),
            bg='white'
        )
        self.smiley_label.pack(pady=10)
        
        # Spread info labels
        self.spread_value_label = ttk.Label(main_frame, text="Spread: --", font=('Arial', 10))
        self.spread_value_label.pack(pady=5)
        
        self.spread_percent_label = ttk.Label(main_frame, text="Percent: --%", font=('Arial', 10))
        self.spread_percent_label.pack(pady=5)
        
        # Contracts info
        contracts_text = f"{self.spread_contracts[0]} vs {self.spread_contracts[1]}"
        contracts_label = ttk.Label(main_frame, text=contracts_text, font=('Arial', 9))
        contracts_label.pack(pady=5)
        
        # Legend
        legend_frame = ttk.LabelFrame(main_frame, text="Legend")
        legend_frame.pack(fill='x', pady=10)
        
        ttk.Label(legend_frame, text="😊 = Positive Spread (Green)", foreground='green').pack(pady=2)
        ttk.Label(legend_frame, text="☹️ = Negative Spread (Red)", foreground='red').pack(pady=2)
        
        # Close button
        ttk.Button(main_frame, text="Close", command=self.stop_spread_monitoring).pack(pady=10)
        
        # Handle window close
        self.spread_smiley_window.protocol("WM_DELETE_WINDOW", self.stop_spread_monitoring)

    def update_spread_smiley(self, spread_value, spread_percentage):
        """Update smiley based on spread value"""
        if not self.spread_smiley_window or not self.spread_smiley_window.winfo_exists():
            return
        
        def update_gui():
            try:
                # Update smiley based on spread
                if spread_value > 0:
                    # Positive spread - Happy face (Green)
                    self.smiley_label.config(text="😊", fg='green')
                    self.spread_smiley_window.configure(bg='light green')
                else:
                    # Negative spread - Sad face (Red)
                    self.smiley_label.config(text="☹️", fg='red')
                    self.spread_smiley_window.configure(bg='light coral')
                
                # Update spread values
                self.spread_value_label.config(text=f"Spread: {spread_value:+.2f}")
                self.spread_percent_label.config(text=f"Percent: {spread_percentage:+.2f}%")
                
                # Blink effect for significant changes
                if abs(spread_percentage) > 1.0:  # More than 1% change
                    current_bg = self.smiley_label.cget('bg')
                    self.smiley_label.config(bg='yellow' if current_bg == 'white' else 'white')
                    self.spread_smiley_window.after(500, lambda: self.smiley_label.config(bg='white'))
                
            except Exception as e:
                print(f"Error updating smiley: {e}")
        
        self.spread_smiley_window.after(0, update_gui)
    ...
    def setup_spread_analysis_tab(self, notebook):
        """Add a dedicated tab for spread analysis"""
        spread_frame = ttk.Frame(notebook)
        notebook.add(spread_frame, text="Spread Analysis")
        
        # Configuration frame
        config_frame = ttk.LabelFrame(spread_frame, text="Spread Configuration")
        config_frame.pack(fill='x', padx=10, pady=10)
        
        # Symbol selection
        ttk.Label(config_frame, text="Commodity:").grid(row=0, column=0, padx=5, pady=5, sticky='w')
        self.spread_commodity = ttk.Combobox(config_frame, values=["GOLD", "SILVER", "CRUDEOIL", "NATURALGAS"])
        self.spread_commodity.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.spread_commodity.set("GOLD")
        
        # Contract selection
        ttk.Label(config_frame, text="Contract 1:").grid(row=1, column=0, padx=5, pady=5, sticky='w')
        self.spread_contract1_combo = ttk.Combobox(config_frame)
        self.spread_contract1_combo.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        
        ttk.Label(config_frame, text="Contract 2:").grid(row=2, column=0, padx=5, pady=5, sticky='w')
        self.spread_contract2_combo = ttk.Combobox(config_frame)
        self.spread_contract2_combo.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        
        # Load contracts button
        ttk.Button(config_frame, text="Load Contracts", 
                  command=self.load_spread_contracts).grid(row=0, column=2, rowspan=3, padx=10, pady=5)
        
        # Thresholds
        ttk.Label(config_frame, text="Positive Threshold:").grid(row=3, column=0, padx=5, pady=5, sticky='w')
        self.positive_threshold = ttk.Entry(config_frame, width=10)
        self.positive_threshold.grid(row=3, column=1, padx=5, pady=5, sticky='w')
        self.positive_threshold.insert(0, "0.5")  # 0.5% threshold
        
        ttk.Label(config_frame, text="Negative Threshold:").grid(row=4, column=0, padx=5, pady=5, sticky='w')
        self.negative_threshold = ttk.Entry(config_frame, width=10)
        self.negative_threshold.grid(row=4, column=1, padx=5, pady=5, sticky='w')
        self.negative_threshold.insert(0, "-0.5")  # -0.5% threshold
        ...
        ttk.Button(button_frame, text="Start Monitoring", 
                  command=self.start_spread_analysis).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Stop Monitoring", 
                  command=self.stop_spread_analysis).pack(side='left', padx=5)
        ttk.Button(button_frame, text="Show Smiley", 
                  command=self.show_advanced_smiley).pack(side='left', padx=5)
        
        # Spread history table
        history_frame = ttk.LabelFrame(spread_frame, text="Spread History")
        history_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        # Treeview for spread history
        columns = ('Time', 'Contract1', 'Contract2', 'Price1', 'Price2', 'Spread', 'Percent', 'Signal')
        self.spread_tree = ttk.Treeview(history_frame, columns=columns, show='headings', height=15)
        
        for col in columns:
            self.spread_tree.heading(col, text=col)
            self.spread_tree.column(col, width=100)
        
        tree_scroll = ttk.Scrollbar(history_frame, orient='vertical', command=self.spread_tree.yview)
        self.spread_tree.configure(yscrollcommand=tree_scroll.set)
        
        self.spread_tree.pack(side='left', fill='both', expand=True)
        tree_scroll.pack(side='right', fill='y')
        
        # Configure grid weights
        config_frame.columnconfigure(1, weight=1)

    def load_spread_contracts(self):
        """Load available contracts for spread analysis"""
        commodity = self.spread_commodity.get()
        contracts = self.get_monthly_contracts(commodity)
        
        if contracts:
            self.spread_contract1_combo['values'] = contracts
            self.spread_contract2_combo['values'] = contracts
            
            if len(contracts) >= 2:
                self.spread_contract1_combo.set(contracts[0])
                self.spread_contract2_combo.set(contracts[1])

    def start_spread_analysis(self):
        """Start advanced spread analysis"""
        contract1 = self.spread_contract1_combo.get()
        contract2 = self.spread_contract2_combo.get()
        
        if not contract1 or not contract2:
            messagebox.showerror("Error", "Please select both contracts")
            return
        
        if contract1 == contract2:
            messagebox.showerror("Error", "Please select different contracts")
            return
        
        self.spread_contracts = [contract1, contract2]
        self.spread_monitor_running = True
        
        # Start monitoring thread
        threading.Thread(target=self.advanced_spread_monitoring, daemon=True).start()
        
        self.log_message(f"Started advanced spread analysis: {contract1} vs {contract2}")

    def stop_spread_analysis(self):
        """Stop advanced spread analysis"""
        self.spread_monitor_running = False
        self.log_message("Stopped spread analysis")

    def advanced_spread_monitoring(self):
        """Advanced spread monitoring with thresholds"""
        ...
        while self.spread_monitor_running and self.is_logged_in:
            try:
                ...
                if spread_data:
                    # Add to history
                    self.add_spread_to_history(spread_data)
                    
                    # Check thresholds
                    try:
                        pos_threshold = float(self.positive_threshold.get())
                        neg_threshold = float(self.negative_threshold.get())
                        
                        if spread_data['spread_percentage'] >= pos_threshold:
                            self.notify_spread_signal(spread_data, "POSITIVE")
                        elif spread_data['spread_percentage'] <= neg_threshold:
                            self.notify_spread_signal(spread_data, "NEGATIVE")
                    except ValueError:
                        pass  # Threshold values invalid
                
                time.sleep(update_interval)
                
            except Exception as e:
                self.log_message(f"Error in advanced spread monitoring: {e}")
                time.sleep(5)

    def add_spread_to_history(self, spread_data):
        """Add spread data to history tree"""
        def update_gui():
            # Determine signal type
            signal = "NEUTRAL"
            if spread_data['spread'] > 0:
                signal = "POSITIVE"
            elif spread_data['spread'] < 0:
                signal = "NEGATIVE"
            
            # Add to treeview
            self.spread_tree.insert('', 0, values=(
                spread_data['timestamp'].strftime("%H:%M:%S"),
                spread_data['contract1'],
                spread_data['contract2'],
                f"{spread_data['price1']:.2f}",
                f"{spread_data['price2']:.2f}",
            ...
            if len(self.spread_tree.get_children()) > 50:
                self.spread_tree.delete(self.spread_tree.get_children()[-1])
        
        self.root.after(0, update_gui)

    def show_advanced_smiley(self):
        """Show advanced smiley window"""
        if not self.spread_contracts:
            messagebox.showerror("Error", "Please start spread monitoring first")
            return
        
        self.show_spread_smiley_window()

    def notify_spread_signal(self, spread_data, signal_type):
        """Show notification for spread signals"""
        # Play sound for significant signals
        if abs(spread_data['spread_percentage']) > 1.0:
            try:
                import winsound
                frequency = 1000  # Hz
        ...
        if signal_type == "POSITIVE":
            title = "📈 Positive Spread Detected!"
            message = f"{spread_data['contract1']}-{spread_data['contract2']}: +{spread_data['spread_percentage']:.2f}%"
            color = 'green'
        else:
        ...
        # Log the alert
        self.log_message(f"ALERT: {title} {message}")

    # ============= END SPREAD MONITORING FUNCTIONS =============
